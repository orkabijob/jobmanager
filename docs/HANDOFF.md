...
- **F15 — manual "graduate"** — `EnrollmentService.CompleteAsync` (Active→Completed, guards non-Active) + a "סיום" button + "הושלם" badge on the class roster. Decision: completion is a manual CS action, not auto-on-syllabus. Also guarded `ToggleAsync` so the tryout pill can't un-complete a completed enrollment. No migration.
- **F12 — client profile** — `/People/Clients/Details/{id}` `[CsOrAdmin]`: client info + all enrollments (class/school/status/payment flags, full history) via `EnrollmentService.ListByClientAsync`. Client name links to it. No migration.
- **F13 — attendance history** — `/Attendance/History` `[CsOrAdmin]`: recent lesson logs + present/absent/total counts (`SchedulingService.ListLessonHistoryAsync`); CS-dashboard card. No migration.
