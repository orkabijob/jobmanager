...
**Cost:** ~$0/mo (Neon free + free app host). Accepted tradeoff: morning cold start; daily jobs run via the in-process scheduler's catch-up-on-wake (no external cron, no keep-warm).
...
├─ Jobs/            DailyAutomations BackgroundService + outbox drainer (in-process; catch-up-on-wake; Slice 4)
...
**ShiftInstanceGenerator strategy:** rolling **~30-day horizon** generated by the daily scheduler run, plus **on-demand regeneration** when a template changes (already-detached/edited instances preserved). Avoids materializing a whole year up front while keeping instances available.
...
- **Slice 0 — Walking skeleton (deploy day one).** Free host + Neon + migrations-on-boot + Identity (email/password **and** Google) + 4 roles + role-routed dashboards + **archival global query filter** + audit interceptor + **RTL Liquid-Glass base layout** + `/health` + the `/api/*` 401 seam + the env-var admin seed. De-risks the integration seams (Google callback, Neon pooler, RTL-vs-glass, migrations on the host) before they multiply. (The job scheduler is in-process and lands in **Slice 4** — there is no Slice-0 cron endpoint.)
...
- **End-to-end verification** per slice: deploy to the real free host, exercise the slice's primary path in Hebrew RTL on a mobile viewport (Playwright available), confirm the in-process scheduler fires a daily job idempotently, and confirm archived data does not leak into current-year views.
